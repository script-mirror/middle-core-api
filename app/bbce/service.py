...
class ProdutoInteresse:
    ...
    def post(produtos_interesse: List[ProdutoInteresseSchema]):
        ...
        df_produtos_interesse = pd.DataFrame(Produto.get_ids_produtos_by_str_produtos([
                                             prod["str_produto"] for prod in records]))

        df = df_produtos_interesse.merge(df_produtos_ordem)[
            ["id_produto", "ordem"]]

        __DB__.db_execute(ProdutoInteresse.tb.delete(), commit=prod)

        __DB__.db_execute(ProdutoInteresse.tb.insert().values(
            df.to_dict("records")), commit=prod)
        return df.to_dict("records")
...
class Negociacoes:
    ...
    def get_datahora_ultima_negociacao(categoria_negociacao: Optional[CategoriaNegociacaoEnum] = None):
        ...
        if categoria_negociacao:
            query = query.join(
                CategoriaNegociacao.tb, CategoriaNegociacao.tb.c[
                    "id"] == Negociacoes.tb.c["id_categoria_negociacao"]
            ).where(
...
class NegociacoesResumo:
    ...
    def get_negociacao_bbce(produto: str, categoria_negociacao: Optional[CategoriaNegociacaoEnum]):
        ...
        if categoria_negociacao:
            query = query.join(
                ...
                CategoriaNegociacao.tb, CategoriaNegociacao.tb.c[
                    "id"] == NegociacoesResumo.tb.c["id_categoria_negociacao"]
            ).where(db.and_(
        ...
        df = pd.DataFrame(result, columns=[
                          'date', 'open', 'high', 'low', 'close', 'volume', 'preco_medio'])
    ...
    def spread_preco_medio_negociacoes(produto1: str, produto2: str, categoria_negociacao: Optional[CategoriaNegociacaoEnum]):
        __DB__.connect()
        resposta1 = NegociacoesResumo.get_negociacao_bbce(
            produto1, categoria_negociacao)
        resposta2 = NegociacoesResumo.get_negociacao_bbce(
            produto2, categoria_negociacao)
        ...
        df_combinado['spread'] = df_combinado['preco_medio_2'] - \
            df_combinado['preco_medio_1']
    ...
    def get_negociacoes_fechamento_interesse_por_data(data: datetime.date, categoria_negociacao: Optional[CategoriaNegociacaoEnum] = None):
        ...
        df_produtos_interesse = pd.DataFrame(ProdutoInteresse.get_all())[
            ["str_produto", "id"]]
        df_produtos_interesse = df_produtos_interesse.rename(
            columns={'str_produto': 'produto'})

        cte = db.select(
            Produto.tb.c['str_produto'],
            NegociacoesResumo.tb.c['id_produto'],
            db.func.timestamp(
                NegociacoesResumo.tb.c['data'], NegociacoesResumo.tb.c['hora_fechamento']),
            NegociacoesResumo.tb.c['preco_fechamento'],
            ...
            cte = (
                cte.join(
                    ...
                    CategoriaNegociacao.tb, CategoriaNegociacao.tb.c[
                        ...
                        Produto.tb.c['str_produto'].in_(
                            df_produtos_interesse['produto'].tolist())
                        )).cte('cte_groups')
            )
        else:
            cte = (
        ...
        result = __DB__.db_execute(query, commit=prod)
        df = pd.DataFrame(result, columns=[
                          'produto', 'id_produto', 'datetime_fechamento', 'preco_fechamento', 'rn'])
        df['datetime_fechamento'] = pd.to_datetime(
            df['datetime_fechamento']).dt.strftime('%Y-%m-%d %H:%M')

        df = df.round(2)

        df = pd.merge(df_produtos_interesse, df, on='produto', how='left')
        df['produto'] = df['produto'].str.extract(
            r'SE CON (.{1,}) - Preço Fixo')
    ...
    def get_negociacoes_interesse_por_data(data: datetime.date, categoria_negociacao: Optional[CategoriaNegociacaoEnum] = None):
        ...
        df = pd.DataFrame(result, columns=['produto', 'date', 'open', 'high',
                          'low', 'close', 'volume', 'preco_medio', 'total', 'hora_fechamento'])
        df['date'] = pd.to_datetime(df['date']).dt.strftime('%Y-%m-%d')

        df_fechamento_anterior = pd.DataFrame(NegociacoesResumo.get_negociacoes_fechamento_interesse_por_data(
            data-datetime.timedelta(days=1), categoria_negociacao))
        df_fechamento_anterior.columns = [
            'produto', 'preco_fechamento_anterior', 'datetime_fechamento_anterior']
        ...
        df['change_percent'] = (
            (df['close'] / df['preco_fechamento_anterior']) * 100) - 100
...
*{
  font-weight: bold!important;
        ...
        if not categoria_negociacao:
            categorias = [CategoriaNegociacaoEnum(
                "Mesa"), CategoriaNegociacaoEnum("Boleta Eletronica")]
        else:
            categorias = [categoria_negociacao]
        for categoria in categorias:
            df = pd.DataFrame(
                NegociacoesResumo.get_negociacoes_interesse_por_data(data, categoria))
            data_hora_ultima_negociacao = Negociacoes.get_datahora_ultima_negociacao(
                categoria)
            df["datetime_fechamento_anterior"] = pd.to_datetime(
                df['datetime_fechamento_anterior']).dt.strftime("%d/%m")
            df = df.fillna("-")

            df["datetime_fechamento_anterior"]

            df["hora_fechamento"] = df["hora_fechamento"].astype(str).str[:-3]
            df[['granularidade', 'produto']] = df['produto'].str.split(
                ' ', n=1, expand=True)
            df = df[["granularidade", "produto", "preco_fechamento_anterior", "datetime_fechamento_anterior",
                     "close", "hora_fechamento", "volume", "total", "change_value", "change_percent", "open", "high", "low"]]
            ...
            for i, row in df.iterrows():
                ...
                classe = "" if type(
                    row["change_value"]) == str else "verde" if row["change_value"] > 0 else "vermelho"
                # formantando change value na para posicionar o sinal de - corretamente
                change_value = "R$-" if type(row["change_value"]) == str else "R$"+str(
                    row["change_value"]) if row["change_value"] > 0 else "-R$"+str(abs(row["change_value"]))
