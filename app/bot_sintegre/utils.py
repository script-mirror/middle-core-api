...
class ElecData:
	...
	def __init__(self, date):
		...
		if date > self.first_day_of_month:
			...
			self.first_day_of_year = get_last_saturday(datetime.date(aux_date.year, 1, 1))

			# Ultimo dia do ano eletrico
			self.last_day_of_year = get_last_friday(datetime.date(aux_date.year, 12, 31))

			# primeiro dia do mes eletrico
			self.first_day_of_month = get_last_saturday(datetime.date(aux_date.year, aux_date.month, 1))
		else:
			self.first_day_of_year = get_last_saturday(datetime.date(self.date.year, 1, 1))
		...
		self.start_of_week = self.first_day_of_month + datetime.timedelta(days=7*self.current_revision)
		
		# Numero de semanas ate a data passada por parametro
		self.num_weeks = count_elec_week(self.first_day_of_year, get_last_saturday(self.date)) + 1	# adicionado 1 para incluir a semana atual
		...
		self.num_weeks_year = count_elec_week(self.first_day_of_year, self.last_day_of_year + datetime.timedelta(days=1))

		# Mes eletrico da data passada por parametro
		self.month_reference = (self.first_day_of_month + datetime.timedelta(days=6)).month
