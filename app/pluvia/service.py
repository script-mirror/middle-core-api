    ...
    def query_ultimo_id_pluvia_df(dt_rodada, flag_pzerada):
        ...
        answer = __DB__.db_execute(
            db.select(
                ...
                db.and_(PluviaEna.tb.c['ID_RODADA'] ==
                        id_rodada_pluvia, PluviaEna.tb.c['CD_BACIA'] == 21)
            )
        )
        df_rodada_pluvia = pd.DataFrame(answer.fetchall(), columns=[
                                        'dt_referente', 'vl_ena', 'cd_bacia'])
        df_bacias = pd.DataFrame(ons_service.BaciasSegmentadas.get_bacias_segmentadas_by_cd_bacia(
            21), columns=['cd_bacia', 'cd_submercado'])

        df_rodada_pluvia = pd.merge(
            df_rodada_pluvia, df_bacias, on=['cd_bacia'])

        df_rodada_pluvia['str_submercado'] = df_rodada_pluvia.apply(
            lambda x: "SE" if x['cd_submercado'] == 1 else "S" if x['cd_submercado'] == 2 else "NE" if x['cd_submercado'] == 3 else "N", axis=1)

        df_rodada_pluvia['dt_referente'] = pd.to_datetime(
            df_rodada_pluvia['dt_referente'], format="%Y/%m/%d")
