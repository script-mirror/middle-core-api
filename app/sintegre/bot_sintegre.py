...
def download_products(driver: webdriver.Chrome, products: List[dict], product_date: datetime.date) -> Dict[str, List[dict]]:
    ...
    for product in products:
        ...
        try:
            ...
            if '.txt' in url:
                time.sleep(1)
                txt: str = driver.page_source[driver.page_source.index(
                    'pre-wrap;">')+11:driver.page_source.index('</pre></body></html>')]
...
def send_to_webhook(airflow_products: List[str]) -> None:
    for product in airflow_products:
        ...
        is_new = requests.post("http://localhost:8000/api/v2/bot-sintegre/verify", json={
                               "nome": product['product_details']['nome'], "fileHash": file_hash}).json()
    ...
    for prod in products:
        ...
        for date_diff in prod['dateDiff']:
            if prod['dateDiffUnit'] == 'day':
                date_params.append(
                    product_date + datetime.timedelta(days=date_diff))
            elif prod['dateDiffUnit'] == 'month':
                date_params.append(product_date.replace(
                    day=15) + datetime.timedelta(days=(30 * date_diff)))
        url_params = [x.strftime(prod['datePattern']).encode(
            'utf-8').decode('utf-8') if "{" not in prod['datePattern'] else elec_date_to_str(x, prod['datePattern']) for x in date_params]
        url_params = [x.upper() if "%b" in prod['datePattern']
                      else x for x in url_params]
...
def trigger_bot(product_date: datetime.date, product_id: int = None, trigger_webhook: bool = True, download_timeout: int = 300) -> None:
    ...
    produtos = get_url_datetime_pattern(
        get_products(product_id), product_date=product_date)

    logger.info("Iniciando downloads")

    airflow_products: List[dict] = download_products(
        driver, produtos, product_date=product_date)
    products_to_remove = []
    ...
    while not is_download_complete("/tmp"):
        logger.info(
            f"Download em andamento... {time.time() - start_time:.2f}s")

        if time.time() - start_time > download_timeout:
            raise Exception(
                "O download não foi concluído dentro do tempo limite.")
        time.sleep(1)
    driver.quit()
